...
namespace CSharpTest.Net.BPlusTree.Test
{
    ...
    public class TestBulkInsert
    {
        ...
        private static IEnumerable<IEnumerable<KeyValuePair<int, string>>> CreateSets(int count, int size,
            Dictionary<int, string> expect)
        {
        ...
        private void TestMergeSequenceInFile(BPlusTreeOptions<int, string> options, int count)
        {
        ...
        public void TestMergeSequenceInFile()
        {
            ...
            using (TempFile temp = new TempFile())
            {
                ...
                TestMergeSequenceInFile(options.Clone(),
                    options.MaximumValueNodes * options.MaximumChildNodes * options.MaximumChildNodes);
                TestMergeSequenceInFile(options.Clone(),
        ...
        public void TestMergeSortedEnumerations()
        {
            ...
            foreach (KeyValuePair<int, string> pair in
                OrderedKeyValuePairs<int, string>.Merge(Options.KeyComparer, DuplicateHandling.LastValueWins,
                    sets.ToArray()))
            {
                string val;
                Assert.IsTrue(test.TryGetValue(pair.Key, out val));
