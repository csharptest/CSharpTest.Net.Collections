...
namespace CSharpTest.Net.Collections
{
    ...
    public class TransactionLogOptions<TKey, TValue>
    {
        ...
        ///     Options used to initialize a TransactionLog
        /// </summary>
        public TransactionLogOptions(string fileName, ISerializer<TKey> keySerializer,
            ISerializer<TValue> valueSerializer)
        {
            FileName = Check.NotEmpty(fileName);
    ...
    public class TransactionLog<TKey, TValue> : ITransactionLog<TKey, TValue>
    {
        ...
        ///     Returns the current size of the log file in bytes
        /// </summary>
        public long Size => _logfile != null
            ? _fLength
        ...
        ///     Merges the contents of the log with an existing ordered key/value pair collection.
        /// </summary>
        public IEnumerable<KeyValuePair<TKey, TValue>> MergeLog(IComparer<TKey> keyComparer,
            IEnumerable<KeyValuePair<TKey, TValue>> existing)
        {
        ...
        ///     Replay the log file from the position provided and output the new log position
        /// </summary>
        private IEnumerable<LogEntry> EnumerateLog(long[] position)
        {
            lock (_logSync)
            {
                long pos = 0;
                ...
                using (Stream io = new FileStream(_options.FileName, FileMode.Open, FileAccess.Read,
                    FileShare.ReadWrite, 0x10000, FileOptions.SequentialScan))
                {
                    bool valid = true;
                    ...
                    while (valid && (pos = position[0] = io.Position) + minSize < length)
                    {
                        ...
                        while (opCount-- > 0)
                        {
                            ...
                            if (entry.OpCode != OperationCode.Add && entry.OpCode != OperationCode.Update &&
                                entry.OpCode != OperationCode.Remove)
                            {
                                valid = false;
                                break;
                            }
        ...
        private void TruncateLog(long position)
        {
            ...
            {
                Close();
                using (Stream io = new FileStream(_options.FileName, FileMode.OpenOrCreate, FileAccess.Write,
                    FileShare.Read))
                {
