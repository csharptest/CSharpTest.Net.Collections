...
namespace CSharpTest.Net.Collections
{
    partial class BPlusTree<TKey, TValue>
    {
            ...
            using (NodeTransaction trans = _storage.BeginTransaction())
            {
                ...
                if (small.Ptr.Count + big.Ptr.Count <= fillFactor ||
                    ...
                    using (NodePin newBig = trans.Create(parent, big.Ptr.IsLeaf))
                    {
                        ...
                        CopyElements(small.Ptr, 0, newSmall.Ptr, 0, Math.Min(borrowing, small.Ptr.Count),
                            small.Ptr[0].Key);
                        CopyElements(big.Ptr, 0, newSmall.Ptr, newSmall.Ptr.Count, borrowing - small.Ptr.Count,
                            bigZeroKey);

                        CopyElements(small.Ptr, borrowing, newBig.Ptr, newBig.Ptr.Count, small.Ptr.Count - borrowing,
                            default(TKey));
                        CopyElements(big.Ptr, Math.Max(0, borrowing - small.Ptr.Count), newBig.Ptr, newBig.Ptr.Count,
                            Math.Min(big.Ptr.Count, total - borrowing), bigZeroKey);
        ...
        private struct RemoveAlways : IRemoveValue<TKey, TValue>
        {
        ...
        private struct RemoveIfValue : IRemoveValue<TKey, TValue>
        {
        ...
        private struct RemoveIfPredicate : IRemoveValue<TKey, TValue>
        {
