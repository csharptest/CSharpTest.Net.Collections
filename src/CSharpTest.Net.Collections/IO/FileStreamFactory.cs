...
namespace CSharpTest.Net.IO
{
    ...
    public class FileStreamFactory : IFactory<Stream>
    {
        ...
        public FileStreamFactory(string filename, FileMode mode)
            : this(filename, mode, mode == FileMode.Append ? FileAccess.Write : FileAccess.ReadWrite, FileShare.None,
                DefaultBuffer, FileOptions.None)
        {
        }

        /// <summary> Creates an IFactory for creating streams on a single file </summary>
        public FileStreamFactory(string filename, FileMode mode, FileAccess access)
            : this(filename, mode, access, access == FileAccess.Read ? FileShare.Read : FileShare.None, DefaultBuffer,
                FileOptions.None)
        {
        }
        ...
        {
            return new FileStream(FileName, _mode, _access, _share, _bufferSize, _options);
        }
    }
}