...
namespace CSharpTest.Net.IO
{
    ...
    public class FragmentedFile : IDisposable
    {
        ...
        public FragmentedFile(string filename, int blockSize, int growthRate, int cacheLimit, FileOptions options)
            : this(
                new FileStreamFactory(filename, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite, 8, options),
        ...
        /// <param name="options">The file options to use when opening the file</param>
        public FragmentedFile(string filename, int blockSize, int growthRate, int cacheLimit, FileAccess access,
        ...
        public long FirstIdentity => _blockSize | 1;
        ...
        ///     Creates a new file (or truncates an existing one) that stores multiple smaller files
        /// </summary>
        public static FragmentedFile CreateNew(string filename, int blockSize, int growthRate, int cacheLimit,
            FileOptions options)
        {
        ...
        ///     Creates a new file (or truncates an existing one) that stores multiple smaller files
        /// </summary>
        public static FragmentedFile CreateNew(IFactory<Stream> streamFactory, int blockSize, int growthRate,
            int cacheLimit)
        {
        ...
        {
            return (s.Length - 1) / _blockSize * _blockSize;
        }
        ...
        private long AllocBlock(FileBlock block, BlockFlags type)
        {
            using (new SafeLock(_syncFreeBlock))
            {
                ...
                if ((block.BlockId & _maskOffset) != (blockId & _maskOffset) ||
                    (block.Flags & BlockFlags.BlockDeleted) == 0)
                    throw new InvalidDataException();
        ...
        private class BlockStreamReader : Stream
        {
            ...
            private bool PrepareRead()
            {
                ...
                {
                    _file.ReadBlock(_block.NextBlockId, _block, _file._blockSize,
                        _validated ? BlockFlags.InternalBlock : 0, _validated);
        ...
        private class BlockStreamWriter : Stream, ITransactable
        {
            private readonly FragmentedFile _file;

            private readonly FileBlock _first, _restore;
            private readonly bool _isNew;

            private Crc32 _checksum;
            private FileBlock _current, _temp;
            private bool _saved, _reverted;
            ...
            private int FileBlockDataSize => _file._blockSize - FileBlock.HeaderSize;

            public override bool CanRead => false;
            public override bool CanSeek => false;
            public override bool CanWrite => !_saved && !_reverted;
        ...
        private class FileBlock : IDisposable
        {
            ...
            public int DataOffset => _baseOffset + HeaderSize;
            private int BlockSize => _alignedIo ? BlockData.Length / 2 : BlockData.Length;

            public long BlockId
            {
                get => ReadLong(0);
                set => WriteLong(0, value);
            }

            public long NextBlockId
            {
                get => ReadLong(8);
                set => WriteLong(8, value);
            }
            ...
            {
                get => (BlockFlags) ReadInt(28);
                set => WriteInt(28, (int) value);
            }
