        ...
        private void SequencedTest(int start, int incr, int stop, string name)
        {
            ...
            using (BPlusTree<int, string> data = new BPlusTree<int, string>(Options))
            {
                ...
                for (int i = start; i != stop; i += incr)
                    if (!data.TryAdd(i, myTestValue1)) throw new Exception();
                ...
                for (int i = start; i != stop; i += incr)
                    if (!data.TryGetValue(i, out test) || test != myTestValue1) throw new Exception();
                ...
                for (int i = start; i != stop; i += incr)
                    if (!data.TryUpdate(i, myTestValue2)) throw new Exception();
                ...
                for (int i = start; i != stop; i += incr)
                    if (!data.TryGetValue(i, out test) || test != myTestValue2) throw new Exception();
                ...
                for (int i = start; i != stop; i += incr)
                    if (data.TryGetValue(i, out test)) throw new Exception();
            }
        }
